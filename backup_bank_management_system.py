...
def delete():
    ...
    with open("bank.csv",newline="") as f:
      reader=csv.reader(f)
      speaker.say("Enter the account number of the account holder you wish to remove from file..")
      speaker.runAndWait()
      number=input("Enter the account number of the account holder you wish to remove from file: ")
      for row in reader:
        if row[0]!=number:
...
while True:
    ...
    if ch==1 or ch=='input data' or ch=='Input Data' or ch=='INPUT DATA' or ch=='inputdata' or ch=='INPUTDATA' or ch=='input & write data' or ch=='Input & Write Data' or ch=='INPUT&WRITEDATA' or ch=='input and write data':
        ...
        delete()
    elif ch==4 or ch=='quit' or ch=='QUIT' or ch=='exit' or ch=='EXIT' or ch=='end' or ch=='END':
