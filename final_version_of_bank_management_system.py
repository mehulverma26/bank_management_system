...
def data():
    ...
    speaker.say(
        "Please enter the details for the account number, account holder and its amount.."
    )
    speaker.runAndWait()
...
def withdraw():
    ...
    try:
        a = pd.read_csv("bank.csv")
        speaker.say(
            "Please enter the account number and the amount that is being withdrawn.."
        )
        speaker.runAndWait()
...
def deposit():
    ...
    try:
        a = pd.read_csv("bank.csv")
        speaker.say(
            "Please enter the account number and the amount that is being deposited.."
        )
        speaker.runAndWait()
        z = int(input("enter the account number: "))
        x = int(input("enter the amount: "))
...
def search():
    ...
    speaker.runAndWait()
    l = int(
        input("Please enter the account number you want to see the details about: ")
    )
    try:
...
if os.path.exists("login.csv"):
    ...
    z = pd.DataFrame(
        {
            "username": ["mehulverma", "mayank", "tanisha", "mansha"],
            "user id": ["Mehul Verma", "Mayank Jain", "Tanisha Saigal", "Mansha"],
            "password": ["26052004", "m1234", "t1234", "ma1234"],
        }
    )
...
def login():
    ...
    speaker.say(
        "To access the bank management system please enter your username and password.."
    )
    speaker.runAndWait()
...
while True:
    ...
    if (
        ...
        ch == "2" or ch == "DISPLAY" or ch == "display" or ch == "data" or ch == "DATA"
    ):
        display()
    elif (
        ch == "3"
        ...
        speaker.runAndWait()
        a = input(
            "Do you want to withdraw or deposit money from the account? : \n 1. withdraw \n 2.deposit"
        )
